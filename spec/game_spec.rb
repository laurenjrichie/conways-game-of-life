...
describe GameOfLife do
  ...
  it 'finding neighbors deals with edge cases' do
    ...
    @game.custom_seed(3,8)
    @game.custom_seed(3,9)
    @game.custom_seed(8,8)
    @game.custom_seed(9,8)

    expect(@game.neighbors(0,0).count("O")).to eq(2)
    expect(@game.neighbors(4,9).count("O")).to eq(2)
    expect(@game.neighbors(9,9).count("O")).to eq(2)
  end
  ...
  it 'moves to the next generation and applies the live or die method to each cell' do
    @game.custom_seed(0,0)
    @game.custom_seed(2,3)
    @game.custom_seed(2,4)
    @game.custom_seed(3,4)
    @game.custom_seed(4,4)
    @game.custom_seed(4,5)
    @game.custom_seed(4,6)
    @game.custom_seed(5,2)
    @game.custom_seed(5,5)
    @game.custom_seed(6,4)
    @game.custom_seed(6,5)

    expect(@game.next_gen).to eq(
      [
        ["-", "-", "-", "-", "-", "-", "-", "-", "-", "-",],
        ["-", "-", "-", "-", "-", "-", "-", "-", "-", "-",],
        ["-", "-", "-", "O", "O", "-", "-", "-", "-", "-",],
  ...
  it 'can deal with successive generations' do
    @game.custom_seed(0,0)
    @game.custom_seed(2,3)
    @game.custom_seed(2,4)
    @game.custom_seed(3,4)
    ...
    third_gen = @game.next_gen

    expect(third_gen).to eq(
      [
        ["-", "-", "-", "-", "-", "-", "-", "-", "-", "-"],
        ["-", "-", "-", "-", "-", "-", "-", "-", "-", "-"],
        ["-", "-", "-", "-", "-", "-", "-", "-", "-", "-"],
